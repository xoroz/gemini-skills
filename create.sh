...
for IMG_NAME in "${!IMAGES[@]}"; do
  ...
  if [ "$MODE" = "MOCKUP" ]; then
    # Use per-image skeleton from .skel/assets/ if available (generated by scripts/gen_mockup_assets.sh)
    SKEL_IMG="$SCRIPT_DIR/.skel/assets/${IMG_NAME}.png"
    if [ -f "$SKEL_IMG" ]; then
      cp "$SKEL_IMG" "$IMG_PATH"
      echo "     ✅ Copied skeleton mockup: ${IMG_NAME}.png"
    else
      # Fallback to generic dummy
      cp "$SCRIPT_DIR/.skel/assets/dummy.png" "$IMG_PATH" 2>/dev/null || touch "$IMG_PATH"
      echo "     ⚠️  No skeleton for ${IMG_NAME}.png — used dummy (run scripts/gen_mockup_assets.sh to fix)"
    fi
